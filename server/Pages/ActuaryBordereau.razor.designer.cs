...
{
    public partial class ActuaryBordereauComponent : ComponentBase, IDisposable
    {
        ...
        IEnumerable<RecoCms6.Models.RecoDb.ServiceProvider> _getFileHandlerList;
        protected IEnumerable<RecoCms6.Models.RecoDb.ServiceProvider> getFileHandlerList
        {
            get
            {
        ...
        int _selectedFileHandler = 0;
        protected int selectedFileHandler
        {
            get
            {
                return _selectedFileHandler;
            }
            set
            {
                if (!object.Equals(_selectedFileHandler, value))
                {
                    var args = new PropertyChangedEventArgs(){ Name = "selectedFileHandler", NewValue = value, OldValue = _selectedFileHandler };
                    _selectedFileHandler = value;
                    OnPropertyChanged(args);
                    Reload();
                }
            }
        ...
        IEnumerable<RecoCms6.Models.RecoDb.FullBordereauReco> _getDataList;
        protected IEnumerable<RecoCms6.Models.RecoDb.FullBordereauReco> getDataList
        {
            get
            {
                return _getDataList;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            {
                var recoDbGetFullBordereauRecosResult = await RecoDb.GetFullBordereauRecos($"{selectedReportDate}");
                getDataList = recoDbGetFullBordereauRecosResult;
            }

            var recoDbGetServiceProvidersResult = await RecoDb.GetServiceProviders(new Query() { Filter = $@"i => i.FileHandler == @0 && i.Active == @1", FilterParameters = new object[] { true, true }, OrderBy = $"Name asc" });
            getFileHandlerList = recoDbGetServiceProvidersResult;
        }
        ...
        {
            filename = "actuarybordereau_" + DateTime.Now.ToString("yyyyMMddTHHmmss");
