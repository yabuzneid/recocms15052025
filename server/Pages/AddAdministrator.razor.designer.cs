...
{
    public partial class AddAdministratorComponent : ComponentBase, IDisposable
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            administrator = new RecoCms6.Models.RecoDb.Administrator(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RecoCms6.Models.RecoDb.Administrator args)
        {
            try
            {
                var recoDbCreateAdministratorResult = await RecoDb.CreateAdministrator(administrator);
                DialogService.Close(administrator);
            }
            catch (System.Exception recoDbCreateAdministratorException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Administrator!" });
            }
        }
