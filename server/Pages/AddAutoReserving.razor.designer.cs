...
{
    public partial class AddAutoReservingComponent : ComponentBase, IDisposable
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            autoreserving = new RecoCms6.Models.RecoDb.AutoReserving(){};

            autoreserving.ClaimOrIncident = false;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RecoCms6.Models.RecoDb.AutoReserving args)
        {
            try
            {
                var recoDbCreateAutoReservingResult = await RecoDb.CreateAutoReserving(autoreserving);
                DialogService.Close(autoreserving);
            }
            catch (System.Exception recoDbCreateAutoReservingException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Unable to create new Auto Reserve",Detail = $"{recoDbCreateAutoReservingException.Message}" });
            }
        }
