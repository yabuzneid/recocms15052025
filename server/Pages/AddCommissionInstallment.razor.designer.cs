...
{
    public partial class AddCommissionInstallmentComponent : ComponentBase, IDisposable
    {
        ...
        {
            commissioninstallment = new RecoCms6.Models.RecoDb.CommissionInstallment(){};

            commissioninstallment.TradeID = TradeID;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RecoCms6.Models.RecoDb.CommissionInstallment args)
        {
            try
            {
                var recoDbCreateCommissionInstallmentResult = await RecoDb.CreateCommissionInstallment(commissioninstallment);
                DialogService.Close(commissioninstallment);
            }
            catch (System.Exception recoDbCreateCommissionInstallmentException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new CommissionInstallment!" });
            }
        }
