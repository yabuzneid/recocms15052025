...
{
    public partial class AddDiaryTemplateComponent : ComponentBase, IDisposable
    {
        ...
        {
            diarytemplate = new RecoCms6.Models.RecoDb.DiaryTemplate(){};
            ...
            var recoDbGetGetMaxDiaryTemplateDisplayOrdersResult = await RecoDb.GetGetMaxDiaryTemplateDisplayOrders();
            diarytemplate.DisplayOrder = recoDbGetGetMaxDiaryTemplateDisplayOrdersResult.FirstOrDefault().MaxDisplayOrder;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RecoCms6.Models.RecoDb.DiaryTemplate args)
        {
            try
            {
                var recoDbCreateDiaryTemplateResult = await RecoDb.CreateDiaryTemplate(diarytemplate);
                DialogService.Close(diarytemplate);
            }
            catch (System.Exception recoDbCreateDiaryTemplateException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new DiaryTemplate!" });
            }
        }
