...
{
    public partial class AddReceiverComponent : ComponentBase, IDisposable
    {
        ...
        RecoCms6.Models.RecoDb.Receiver _receiver;
        protected RecoCms6.Models.RecoDb.Receiver receiver
        {
            get
            {
                return _receiver;
            ...
            set
            {
                if (!object.Equals(_receiver, value))
                {
                    var args = new PropertyChangedEventArgs(){ Name = "receiver", NewValue = value, OldValue = _receiver };
                    _receiver = value;
                    OnPropertyChanged(args);
                    Reload();
                }
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            receiver = new RecoCms6.Models.RecoDb.Receiver(){};

            receiver.ID = Guid.NewGuid();

            receiver.ProvinceID = Globals.defaultProvinceID;
        }

        protected async System.Threading.Tasks.Task Form0Submit(RecoCms6.Models.RecoDb.Receiver args)
        {
            try
            {
                var recoDbCreateReceiverResult = await RecoDb.CreateReceiver(receiver);
                DialogService.Close(receiver);
            }
            catch (System.Exception recoDbCreateReceiverException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Receiver!" });
            }
        }
