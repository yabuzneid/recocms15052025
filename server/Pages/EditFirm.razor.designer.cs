...
{
    public partial class EditFirmComponent : ComponentBase, IDisposable
    {
        ...
        {
            var recoDbGetFirmByIdResult = await RecoDb.GetFirmById(ID);
            firm = recoDbGetFirmByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RecoCms6.Models.RecoDb.Firm args)
        {
            try
            {
                var recoDbUpdateFirmResult = await RecoDb.UpdateFirm(ID, firm);
                DialogService.Close(firm);
            }
            catch (System.Exception recoDbUpdateFirmException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Firm" });
            }
        }
