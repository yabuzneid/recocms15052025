...
{
    public partial class LloydsBordereauComponent : ComponentBase, IDisposable
    {
        ...
        protected string downloadPath
        {
            ...
            set
            {
                if (!object.Equals(_downloadPath, value))
                {
                    var args = new PropertyChangedEventArgs(){ Name = "downloadPath", NewValue = value, OldValue = _downloadPath };
                    _downloadPath = value;
                    OnPropertyChanged(args);
                    Reload();
                }
        ...
        IEnumerable<RecoCms6.Models.RecoDb.RecoLloydsBordereau> _getDataList;
        protected IEnumerable<RecoCms6.Models.RecoDb.RecoLloydsBordereau> getDataList
        {
            get
            {
                return _getDataList;
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            selectedReportDate = new DateTime(DateTime.Today.Year, DateTime.Today.Month, DateTime.DaysInMonth(DateTime.Today.Year, DateTime.Today.Month));;

            hiddenColumns = new List<string>(){ "ClaimID", "OccurrenceID", "TradeID" };

            filename = "lloydsbordereau_" + DateTime.Now.ToString("yyyyMMddTHHmmss") + ".xlsx";
        ...
        {
            var recoDbGetRecoLloydsBordereausResult = await RecoDb.GetRecoLloydsBordereaus($"{selectedReportDate}");
            getDataList = recoDbGetRecoLloydsBordereausResult;
        }
