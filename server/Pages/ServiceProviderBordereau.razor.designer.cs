...
{
    public partial class ServiceProviderBordereauComponent : ComponentBase, IDisposable
    {
        ...
        IEnumerable<RecoCms6.Models.RecoDb.ServiceProviderBordereau> _getDataList;
        protected IEnumerable<RecoCms6.Models.RecoDb.ServiceProviderBordereau> getDataList
        {
            get
            {
                return _getDataList;
        ...
        IEnumerable<RecoCms6.Models.RecoDb.FirmDetail> _getFirmList;
        protected IEnumerable<RecoCms6.Models.RecoDb.FirmDetail> getFirmList
        {
            get
            {
                return _getFirmList;
        ...
        {
            var recoDbGetParameterDetailsResult = await RecoDb.GetParameterDetails(new Query() { Filter = $@"i => i.ParamTypeDesc == @0 || i.ParamTypeDesc == @1 || i.ParamTypeDesc == @2", FilterParameters = new object[] { "ProgramID", "Contract Year", "Page Size" }, OrderBy = $"ParamDesc asc" });
            ...
            PageSizes = recoDbGetParameterDetailsResult.Where(p=>p.ParamTypeDesc=="Page Size");
            ...
            {
                var recoDbGetServiceProviderBordereausResult = await RecoDb.GetServiceProviderBordereaus($"{selectedReportDate}");
                getDataList = recoDbGetServiceProviderBordereausResult;
            }

            var recoDbGetFirmDetailsResult = await RecoDb.GetFirmDetails(new Query() { Filter = $@"i => i.FirmType == @0 && i.FirmID != @1", FilterParameters = new object[] { "Defense Counsel", 0 }, OrderBy = $"Name asc" });
            getFirmList = recoDbGetFirmDetailsResult;
        ...
        {
            filename = "serviceproviderbordereau_" + DateTime.Now.ToString("yyyyMMddTHHmmss");
